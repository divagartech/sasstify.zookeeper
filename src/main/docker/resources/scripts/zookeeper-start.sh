...
DATA_LOG_DIR=$ZK_DATA_LOG_DIR
LOG_DIR=$ZOO_LOG_DIR
CONF_DIR=$ZK_OPT_DIR/conf
CLIENT_PORT=2181
SERVER_PORT=2888
ELECTION_PORT=3888
TICK_TIME=2000
...
Starts a ZooKeeper server based on the supplied options.
    ...
    --data_dir          The directory where the ZooKeeper process will store its
                        snapshots. The default is /var/sasstify/zookeeeper-3.1.6/lib/zookeeper/data.
    --data_log_dir      The directory where the ZooKeeper process will store its
                        write ahead log. The default is
                        /var/sasstify/zookeeeper-3.1.6/lib/zookeeper/data/log.
    --conf_dir          The directoyr where the ZooKeeper process will store its
                        configuration. The default is /opt/sasstify/zookeeper-3.1.6/conf.
...
function create_config() {
    ...
    if [ $SERVERS -gt 1 ]; then
        echo "standaloneEnabled=false" >> $CONFIG_FILE
...
DYNAMIC_CONFIG_FILE="$CONF_DIR/zoo.cfg.dynamic"
...
create_config && create_jvm_props && create_log_props && create_data_dirs && exec zkServer.sh --config $CONF_DIR start-foreground